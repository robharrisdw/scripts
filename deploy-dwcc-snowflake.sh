# What
## Update and use this script to catalog a snowflake database and turn the metadata into RDF (knowledge graph) format
# Steps
...
catalog-snowflake -A -i --a <account> -d <database> \
-n <catalogName> -o "/dwcc-output" -p <port> \
-r <role> -s <server> -u <user>

# help - https://docs.data.world/en/59261-59392-Connect-to-data.html#UUID-28ae3cdc-e53a-8fe6-b8bc-efcc91c7e4eb
...
# dwcc commands
## catalog-snowflake = tell dwcc to catalog a snowflake database
...
## -r = role in snowflake you wish the snowflake user to run as (defaults to Admin)  You can confirm in the snowflake UI
