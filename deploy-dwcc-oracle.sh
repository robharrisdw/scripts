...
--mount type=bind,source=“$(pwd)“/jdbcdrivers,target=/usr/src/dwcc-config/lib \
datadotworld/dwcc:2.36 \
catalog-oracle -A -i -a <account> -d <database> \
-n <catalogName> -o "/dwcc-output" -p <port> \
-r <role> -s <server> -u <user>

# help - https://docs.data.world/en/59261-59392-Connect-to-data.html#UUID-53905695-f3e4-58ce-9ded-4488c717a4ac
...
## --mount 1 = map the existing folder /dwcc-output inside the dwcc docker container to the dwcc-export folder inside the folder I executed this script from
## --mount 2 = map the existing folder /app/log inside the dwcc docker container to the dwcc-export folder inside the folder I executed this script from
## --mount 3 = map the existing folder /user/src/dwcc-config/lib inside the dwcc docker container to a folder locally containing the jdbc driver 

# dwcc commands
## catalog-oracle = tell dwcc to catalog a oracle database
